...
function title() {
	  console.log(chalk.bold.blue(figlet.textSync('MIAWBOT', {
		font: 'Standard',
...
const connectToWhatsApp = async () => {
	...
	lexxy.ev.on('messages.upsert', async m => {
	    ...
	    require('./index')(lexxy, msg, m, setting, store, welcome, left, set_welcome_group, set_left_db, db_respon_list, set_proses, set_done, _afk)
	})
             ...
             for (let key in data.presences) {
               if (data.presences[key].lastKnownPresence === "composing" || data.presences[key].lastKnownPresence === "recording") {
                 if (afk.checkAfkUser(key, _afk)) {
                  ...
                  lexxy.sendMessage(data.id, { text: `@${key.split("@")[0]} berhenti afk, dia sedang ${data.presences[key].lastKnownPresence === "composing" ? "mengetik" : "merekam"}`, mentions: [key] })
                 }
               }
             }
           }
    ...
    lexxy.groupInspectCode = async(jid) => {
           return require('./lib/inspect')(lexxy, jid)
        }
